...
export const prepareStaker = async (
  connection: Connection,
  minStartingSol: number = 1,
): Promise<PrepareStakerResult> => {
  const stakerKeypair = keypairFromLocalFile("testnet-staker.json");
  const staker = new MockWalletAdapter(stakerKeypair);
  let originalBalanceLamports = await connection.getBalance(
    staker.publicKey,
    "finalized",
  );
  if (originalBalanceLamports < minStartingSol * LAMPORTS_PER_SOL) {
    ...
    originalBalanceLamports = await connection.getBalance(
      staker.publicKey,
      "finalized",
    );
  }
...
export const transferStakeAcc = async (
  connection: Connection,
  stakeAccount: PublicKey,
  owner: Keypair,
  newOwner: PublicKey,
) => {
  const transferAuthTxs = [STAKE_AUTHORITY_ENUM, WITHDRAW_AUTHORITY_ENUM].map(
    (authType) =>
 ...
 * @returns
 */
export const cleanupAllStakeAccs = async (
  connection: Connection,
  owner: Keypair,
) => {
  ...
  for (
    let i = 0;
    i < allStakeAccounts.active.length;
    i += MAX_STAKE_DEACTIVATE_IX_PER_TX
  ) {
    const chunk = allStakeAccounts.active.slice(
      i,
      Math.min(
        allStakeAccounts.active.length,
        i + MAX_STAKE_DEACTIVATE_IX_PER_TX,
      ),
    );
  ...
  for (
    let i = 0;
    i < allStakeAccounts.inactive.length;
    i += MAX_STAKE_WITHDRAW_IX_PER_TX
  ) {
    const chunk = allStakeAccounts.inactive.slice(
      i,
      Math.min(
        allStakeAccounts.inactive.length,
        i + MAX_STAKE_WITHDRAW_IX_PER_TX,
      ),
    );
...
const getAllStakeAccounts = async (
  connection: Connection,
  owner: PublicKey,
): Promise<UserStakeAccounts> => {
...
const determineStakeActivation = (
  parsedStakeAccount: ParsedStake,
  currentEpoch: number,
